...
При чтении значения они не отличаются от аналогичных `inner` методов. При записи значения отличаются, они заменяют текст включая текущий элемент.
...
Для того чтобы объединить эти две функции, мы можем создать контроллер.
```js
